...
describe("Set New Arb Status Telos Arbitration Smart Contract Tests", () => {
    ...
    it("set arbitrator status", async () => {
        expect.assertions(1);

        await arbitration.contract.newarbstatus({new_status: 1, arbitrator: "user1"},
          [{
        ...
        expect(arbitrators.find(arb => arb.arb === "user1").arb_status).toEqual(1);  
    })
    
    it("fails if arbitrator not found", async () => {
         await expect(arbitration.contract.newarbstatus({new_status: 1, arbitrator: "user3"},
            [{
    ...
    it("fails if arb term has expired", async () => {
        ...
        await expect(arbitration.contract.newarbstatus({new_status: 1, arbitrator: "user2"},
            [{
     ...
     it("fails if arbitrator status is invalid", async () => {
        ...
        await expect(arbitration.contract.newarbstatus({new_status: 2, arbitrator: "user2"},
            [{
              actor: user2.accountName,
              permission: "active"
    ...
    it("fails to set arbitrator status if user different than arbitrator tries", async () => {
          ...
          await expect(arbitration.contract.newarbstatus({new_status: 1, arbitrator: "user1"},
            [{
              actor: user2.accountName,
              permission: "active"
            }])).rejects.toThrow();
